...
from typing import Optional, Union  # optional denotes something that could be set to None
...
"""An event handler return value witch can trigger an action or switch active handlers.
...
class EventHandler(tcod.event.EventDispatch[ActionOrHandler]):  # subclass of tcod's EventDispatch
    def __init__(self, engine: game.engine.Engine) -> None:
        super().__init__()
        self.engine = engine

    def handle_events(self, event: tcod.event.Event) -> EventHandler:
        """Handle and event, perform any actions, then return the next active event handler."""
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[game.actions.Action]:
        ...
        return None  # I think this is key later, the game don't advance to the next "turn" if no button is pressed
